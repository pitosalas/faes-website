...
class ContentLoader:
    ...
    def preprocess_photos(self, body: str, source_path: Path, content_dir: Path) -> str:
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            if not stripped.startswith(":photo"):
                processed.append(line)
                continue
            processed.append(self.photo_html(stripped, (source_path, idx), content_dir))
        return "\n".join(processed)

    def photo_html(self, raw_line: str, location: tuple, content_dir: Path) -> str:
        ...
        return env.get_template("photo_figure.html").render(
            name=name,
            caption=caption,
            height=height,
            justify=justify,
